...
int get_sw_reset_cnt(int *reset_cnt)
{
	uint start_sector = 0;
	uint sector_count = 0;
	uchar pid;
	...
	if (ret == 0)
		memcpy(reset_cnt, buf, sizeof(int));

	return ret;
}	

int set_sw_reset_cnt(int reset_cnt)
{
	int ret = 0;
	uint start_sector = 0;
	uint sector_count = 0;
...
int clear_reset_cnt_sectors(uint s_sector, uint sector_num)
{
...
int clear_sw_reset_cnt(void)
{
	return clear_reset_cnt_sectors(1, 1);
}
...
int save_crash_dump(void* buf, uint length_byte, uint start_location)
{
	...
	{		
		memset(temp, 0, SECTOR_SIZE);
		memcpy(temp, (char*)buf + buf_offset, (length > SECTOR_SIZE) ? SECTOR_SIZE:length );
...
int plr_print_log_sector(uint lsn, uint page_count, uint loop, uint boot, int (*funcp)(uint, uint,uint,uint))
{
	...
	uint sector_count = 0;
	uint total_sectors = get_dd_total_sector_count();
