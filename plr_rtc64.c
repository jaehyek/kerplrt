...
void rtc64_set_tick_count(u32 tick_base)
{
	...
	if (t1 < 0LL) { 		// If the values are upside down, resample it
		...
		_sec_base		= get_tick2sec(_tick_base);
	}
}
...
u64 rtc64_get_tick_count(void)
{
	u64 t64 = 0;	// result
	u64 tsec;		// tick converted from second since 1970
	...
	while (1)	{
		t64 = (_tick_up << TICK_BITS) | (u64)t32;
		
		tsec = get_sec2tick(s1);	
		if (tsec < (t64 + (1UL << 31))) 
			break;                                                                                                                                        
...
extern const u64 	TICK_RES_NS; 		// uset is nano second
extern const u64 	TICK_RES_NS_INV;	// inverse of TICK_RES_NS

static void test_rtc_64__tick_increment(void)
{
	int i;
	u64 t1, t2;
	u32 msec = 10;
...
static void test_rtc_64__tick_sync(void)
{
	int i;
	u32 s1, s2;
	u64 t1, t2;
	...
	for (i = 0; i < 3000; i++) {
		...
		t2 = get_sec2tick(s1);
		s2 = get_tick2sec(t1);		
		plr_info("(s1, s2), (t1, t2) = (%u, %u), (%llu, %llu)\n", s1, s2, t1, t2);		
