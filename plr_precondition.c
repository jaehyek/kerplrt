...
int check_erase_zero_condition(uint erase_area_start, uint erase_area_length)
{
	...
	plr_info ("check read speed : %lu KB\n", 
...
{	
	...
	// joys,2015.11.03
	//ret = dd_erase_for_poff(erase_area_start, erase_area_length, TYPE_DISCARD);
...
static int make_dirty_condition(uint test_start_sector, uint test_sector_length, int total_sectors)
{
	...
	int dirty_count = 0;
	int word_count = FILL_BLOCK_CNT * 128;		// * int(4byte) = 4MB
	...
	if (g_plr_state.filled_data.type == 2) {
		well512_seed2(g_plr_state.filled_data.value);
	}
	// Fill dirty memory 4MB
	else {
		while (word_count-- > 0) {
			*((uint *)dirty_buff) = (uint)dirty_data;
			dirty_buff += 1;
		}
	...
	if (test_start_sector > 0) {
		...
		for (i = dirty_start; i < dirty_end && sector_cnt<= sectors_cnt_to_fill; i += dirty_count, sector_cnt += dirty_count) {
			...
			if (g_plr_state.filled_data.type == 2) {
				// Fill dirty memory 4MB
				word_count = FILL_BLOCK_CNT * 128;
				while (word_count-- > 0) {
					*((uint *)dirty_buff) = (uint)well512_rand2();
					dirty_buff += 1;
				}
				dirty_buff = (uint*) plr_get_write_buffer();
			}
...
int precondition_by_protocol()
{
	int ret = 0;

	// joys,2015.10.14 for Daxx 0008 read distturb
...
{	
    ...
    plr_info("[%s] SW Reset cnt Clear\n", __func__);
    ret = clear_sw_reset_cnt();
    if( ret < 0 )
    {
        plr_err("[%s] SW Reset cnt Clear err\n", __func__);
    ...
    plr_info("test_type2 %s\n", g_plr_state.test_type2);
	if (IS_AGING_TEST && 
		(g_plr_state.test_num == 8 || (g_plr_state.test_num == 9)))
		return 0;

	if (g_plr_state.total_device_sector < 1) {
		plr_err ("Error!! get emmc block count failed!!\n");
		return -1;
	}
