...
#ifdef PLR_SYSTEM	//KKR 160126 err 처리 추가
	if (len < 0)
		return -1;
#endif
...
static void set_recv_param_to_state(e_send_token sent_token, PPROTOCOL_PARAM p)
{
	...
	switch(sent_token){
		...
		case PLRTOKEN_TEST_INFO:
			...
			g_plr_state.filled_data.type = strtoud(p->params[15]);
			g_plr_state.filled_data.value = strtoud(p->params[16]);
			...
			g_plr_state.extend_feature = strtoud(p->params[22]);

			memset(g_plr_state.user_defined_option, 0, 3 * 20); 
			switch(p->num_of_params - 1)
			{
				case 25:
					strcpy(g_plr_state.user_defined_option[2], p->params[25]);
				case 24:
                    strcpy(g_plr_state.user_defined_option[1], p->params[24]);
				case 23:
					strcpy(g_plr_state.user_defined_option[0], p->params[23]);
				break;   
			}
			break;
		...
		case PLRTOKEN_INIT_INFO:
			...
			g_plr_state.filled_data.type = strtoud(p->params[2]);
			g_plr_state.filled_data.value = strtoud(p->params[3]);
...
static e_receive_token _recv_acks( e_send_token token, char *buf )
{
	int len = 0;
	int i;
#ifdef PLR_SYSTEM	//KKR 160126 retry 추가
	int retry = 10; 
#endif
...
int send_token( e_send_token token, char *buf )
{
	int retry = 100;
	int data_len = 0;
	...
	if(buf)
	{
		char temp[PLRTOKEN_BUFFER_SIZE] = {0,};
		data_len = strlen(buf);
...
#else
			fprintf(stderr, "plrtoken %s %d/%d/%s\n", send_token_string[token], crc, data_len, buf); 
#endif
		}
		else {
#ifndef PLR_SYSTEM
...
int send_token_param( e_send_token token, uint param ) 
{
	...
	return send_token(token, send_buff);
}
