...
//#define MAX_REQUST_SECTORS	32768UL		// 16MB
#define MAX_REQUST_SECTORS	16384UL		// 16MB
...
static int select_random_zone(perf_random_zone_s *ran_zone_info, u32 total_zone)
{
	int i = 0, j = 0;
	int zone_num = 0;
	int random_zone_cnt = ran_zone_info->selected_zone_cnt;
...
static int do_perf_test(uchar *buf, uint start_lsn, u32 test_area_len)
{
	...
	do {
		...
		plr_info_highlight (" | %10u |     %7u |           %7u |           %7u |           %7u |           %7u |\n",
				test_area_len / 2,
				request_len / 2,
