...
static void sequential_write()
{
    ...
    for(;loop<(K_BYTES*3);loop++){
    	...
    	for (j=0, i = start_addr; i < end_addr; i += len, j++) {
		    if ( (i + len) > end_addr )
		    	len = end_addr - i;
	
		    write_buffer(i, buffer, SEQ_WRITE);
	
		    if ((j % 20 == 0) || ((i + len) >= end_addr))
	    		plr_info("\r %u / %u", i + len - 1, end_addr - 1);
    	}printf("\n");
        
        if(loop%1 == 0) printf("sequential write loop count: %d / %d\n", loop, (K_BYTES*3));
   }
}*/
static void sequential_write()
{
 ...
 *----------------------------------------------------------------------------------------
 */
int initialize_dpin_0013( uchar * buf, uint test_area_start, uint test_area_length )
{
	int ret = 0;
    ...
    printf("============ initialize (always) start ==============\n");
	...
	{
        printf("============ initialize (one time) start ==============\n");
#if 1
		/* known pattern write  */
		...
		ret = random_write_and_sleep(RANDOM_LOOP_CNT, SLEEP_DELAY_MSEC);
		if(ret)
...
/* Skip precondition for continue the test which stopped by EMP shock */
		...
		dd_erase_for_poff(start_addr, (end_addr-start_addr), TYPE_SANITIZE);
        printf("============ initialize (one time) end ==============\n");
	}
    printf("============ initialize (always) end ==============\n");
	return ret;
}
