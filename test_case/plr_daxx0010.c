...
#define RD_QE_NUMBER                        (0x00010203UL)
...
static u32* prepare_ref_buffer(void)
{
	...
	u32 *p		= xbuf;
    u32 word   = RD_QE_NUMBER;

	while ( p != xbufe ) {
		*p++ = word;
        word += 0x04040404;
        if(word > 0x7C7D7E7F) word = RD_QE_NUMBER;
	}
...
static int fill_read_disturb_area( void )
{
    ...
    u32 write_type = g_plr_state.filled_data.type;

	delay = 86400*ADDITIONAL_DAYS_PER_1K/(end_addr/wLen)*percentage/100; //24*60*60*ADDITIONAL_DAYS_PER_1K/(end_addr/wLen)*percentage%;
	...
	/* Write pattern */
	while(repeat--) {
		plr_info (" [Cycles : %u / %u] with ", (g_plr_state.finish.value - repeat), (g_plr_state.finish.value));
		
		if(write_type==2){
            plr_info("random_data\n");
            // fill random pattern
...
int initialize_daxx_0010( uchar * buf, uint test_area_start, uint test_area_length )
{
	int ret = 0;
...
int read_daxx_0010(uchar * buf, uint test_area_start, uint test_area_length)
{
   	verify_set_current_step(VERI_EXTRA_STEP);
	return extra_verification(buf, NULL);	// read verify func
}

int write_daxx_0010(uchar * buf, uint test_area_start, uint test_area_length)
{
